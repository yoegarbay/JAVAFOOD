...
async function cargarDatos() {
    ...
    // Clientes registrados (desde la tabla clientes)
    try {
        const res = await fetch("/api/clientes");
        if (res.ok) {
            const clientes = await res.json(); // [{id_cliente, nombre, email}]
            const dl = document.getElementById("dl-clientes");
            clientes.forEach(c => {
                const opt = document.createElement("option");
                opt.value = c.nombre;  // el datalist usa el nombre como valor
                dl.appendChild(opt);
            });
        }
...
function addLinea() {
    ...
    div.innerHTML = `
        ...
        <input type="number" id="cantidad-${n}" value="1" min="1" oninput="recalcular()">
        <input type="number" id="precio-${n}"   step="0.01" min="0" placeholder="€" oninput="recalcular()" readonly>
...
async function crear() {
    ...
    if (items.length === 0 || !valido) {
        mostrarMsg("err", "❌ Selecciona al menos un producto completo.");
        return;
    }
