...
function validarFormulario() {
    // Si hay sesión activa, el nombre viene de sessionStorage — no validar el input
    const clienteSession = JSON.parse(sessionStorage.getItem('javafoodUser') || 'null');
    if (!clienteSession) {
...
async function confirmarPago() {
    ...
    const clienteSession2 = JSON.parse(sessionStorage.getItem('javafoodUser') || 'null');
    const nombreCliente = clienteSession2
        ? (clienteSession2.nom || clienteSession2.nombre || 'Cliente')
    ...
    try {
        // id_cliente viene de clienteSession2 declarado arriba
        ...
        const response = await fetch('/api/pedidos', {
            ...
            body: JSON.stringify({
                ...
                id_cliente:     clienteSession2 ? (clienteSession2.id_cliente || clienteSession2.id || null) : null,
        ...
        // ── SISTEMA DE PUNTOS ──────────────────────────────────────────
        const PTS_POR_EURO = 10; // 10 puntos por cada € gastado en productos normales

        const user2 = JSON.parse(sessionStorage.getItem('javafoodUser') || 'null');
        if (user2) {
            const uid = user2.id_cliente || user2.id;

            // Puntos GASTADOS (promos)
            const puntosGastados = carrito
                .filter(item => item.esPromocion && item.costoPuntos)
                .reduce((acc, item) => acc + item.costoPuntos, 0);

            // Puntos GANADOS (productos normales)
            const totalNormal = carrito
                .filter(item => !item.esPromocion)
                .reduce((acc, item) => acc + item.total, 0);
            const puntosGanados = Math.round(totalNormal * PTS_POR_EURO);

            const delta = puntosGanados - puntosGastados;

            // Solo llamar a la API si hay cambio de puntos
            if (delta !== 0) {
                try {
                    const res = await fetch(`/api/clientes/${uid}/puntos`, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
                    ...
                    if (res.ok) {
                        ...
                        localStorage.setItem('javaFoodPuntos', datos.puntos.toString());
                        console.log(`Puntos actualizados: ${delta > 0 ? '+' : ''}${delta} → saldo: ${datos.puntos}`);
                    }
                } catch (e) {
        ...
        // Guardar items para reseñas en gracias.html
