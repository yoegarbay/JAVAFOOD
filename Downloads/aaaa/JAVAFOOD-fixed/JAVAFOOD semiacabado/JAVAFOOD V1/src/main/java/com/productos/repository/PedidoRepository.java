...
public class PedidoRepository {
    ...
    public PedidoResponse guardarPedido(List<PedidoRequest.LineaPedido> items,
        ...
        try {
            ...
            int idPedido = insertarCabeceraPedido(con, total, nombreCliente, metodoPago, idCliente);
    ...
    public List<PedidoAdminResponse> findByCliente(int idCliente) {
        ...
        try (PreparedStatement st = con.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                lista.add(new PedidoAdminResponse(
                    id,
    ...
    public void updateFull(int id, PedidoEditRequest req) {
        ...
        try {
            con.setAutoCommit(false);
            try (PreparedStatement st = con.prepareStatement(
                    "UPDATE pedidos SET estado=?, nombre_cliente=?, metodo_pago=?, total=? WHERE id_pedido=?")) {
            ...
            try (PreparedStatement st = con.prepareStatement("DELETE FROM pedido_detalle WHERE id_pedido=?")) {
    ...
    public void delete(int id) {
        try (PreparedStatement st = con.prepareStatement("DELETE FROM pedidos WHERE id_pedido=?")) {
            st.setInt(1, id);
            if (st.executeUpdate() == 0) throw new DataAccessException("Pedido #" + id + " no encontrado");
        } catch (SQLException e) { throw new DataAccessException("Error al eliminar pedido #" + id, e); }
    }

    // ── Helper: cabecera del pedido con retry-null para usuarios legacy ────────
    // Intenta guardar con id_cliente. Si hay FK violation (usuario nuevo que no
    // está en tabla clientes), reintenta con id_cliente = null.
    private int insertarCabeceraPedido(Connection con, float total, String nombreCliente,
                                        String metodoPago, Integer idCliente) throws SQLException {
        String sql = "INSERT INTO pedidos (fecha, total, estado, nombre_cliente, metodo_pago, id_cliente) " +
                     "VALUES (NOW(), ?, 'PAGADO', ?, ?, ?)";
        try (PreparedStatement st = con.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            if (!rs.next()) throw new DataAccessException("No se generó id de pedido");
            return rs.getInt(1);
        } catch (SQLIntegrityConstraintViolationException fkEx) {
            // id_cliente no existe en clientes (usuario nuevo) → reintentar con null
            try (PreparedStatement st = con.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                st.setFloat(1, total);
                st.setString(2, nombreCliente);
                st.setString(3, metodoPago);
