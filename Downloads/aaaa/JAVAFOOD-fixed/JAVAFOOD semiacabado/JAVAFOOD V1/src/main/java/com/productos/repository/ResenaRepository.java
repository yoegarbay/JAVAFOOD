...
public class ResenaRepository {
    ...
    public ResenaResponse findByProducto(int idProducto) {
        ...
        String sqlLista = """
            SELECT r.id_resena,
                   COALESCE(u.nom, 'Cliente') AS nombre_cliente,
                   r.puntuacion, r.comentario, r.fecha
            FROM resenas r
            LEFT JOIN usuario u ON u.id = r.id_cliente
            WHERE r.id_producto = ?
    ...
    // ── Puede reseñar: haber comprado el producto Y no haberlo reseñado ya ───
    public boolean puedeResenar(int idCliente, int idProducto) {
        return !yaReseno(idCliente, idProducto) && haPedido(idCliente, idProducto);
    }

    // Verifica que el cliente tenga al menos un pedido con ese producto.
    // Funciona con el FK de pedidos dropeado: id_cliente = usuario.id directamente.
    ...
    // ── Guardar reseña ────────────────────────────────────────────────────────
    // FIX: eliminada la validación haPedido (ver puedeResenar arriba).
    // Solo se comprueba que el usuario no haya reseñado ya este producto.
    ...
    // Busca el nombre en usuario (nuevo sistema), con fallback a clientes (legacy)
    private String getNombreCliente(int idCliente) {
        try (PreparedStatement st = con.prepareStatement(
                "SELECT nom FROM usuario WHERE id = ?")) {
            st.setInt(1, idCliente);
            ResultSet rs = st.executeQuery();
    ...
    // ── ADMIN: crear reseña sin validaciones ──────────────────────────────────
