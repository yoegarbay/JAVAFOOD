...
public class ProductosAdminController {
    ...
    public List<Productos> index() {
        try (Connection con = ds.getConnection()) {
            return new ProductosRepository(con).findAll();
    ...
    public Productos show(@PathVariable int id) {
        try (Connection con = ds.getConnection()) {
            return new ProductosRepository(con).find(id);
        } catch (SQLException e) { throw new DataAccessException(e); }
    }

    @PostMapping
    public Productos store(@RequestBody Productos producto) {
        // FIX: si no se manda stock (llega 0), poner 15 por defecto
        if (producto.getStock() <= 0) producto.setStock(15);

        try (Connection con = ds.getConnection()) {
            new ProductosRepository(con).insert(producto);
            return producto;
        } catch (SQLException e) { throw new DataAccessException(e); }
    }
    ...
    public Productos update(@PathVariable int id, @RequestBody Productos producto) {
        // FIX: misma protección en edición
        if (producto.getStock() <= 0) producto.setStock(15);
    ...
    public void destroy(@PathVariable int id) {
        try (Connection con = ds.getConnection()) {
            new ProductosRepository(con).delete(id);
        } catch (SQLException e) { throw new DataAccessException(e); }
    }
}