...
function vincularBotones() {
    document.addEventListener('click', (e) => {
        if (e.target.tagName === 'BUTTON' && e.target.innerText === '+') {
            ...
            document.getElementById('seccion-extras').style.display = window.location.pathname.includes('Complementos.html') ? 'block' : 'none';
...
function confirmarCompra() {
    ...
    carrito.push({ nombre: nFinal, precio: pFinal, cantidad: cantidad, total: pFinal * cantidad });
...
function actualizarVistaCarrito() {
    ...
    carrito.forEach(item => {
        ...
        contenedor.insertBefore(div, contenedor.querySelector('.total-row') || null);
        total += item.total;
    });
